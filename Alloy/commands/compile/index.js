...
function patchModuleResolution() {
    if (CU[CONST.NODEJS_MODULE_RESOLUTION] && CU[CONST.NODEJS_MODULE_RESOLUTION].enabled) {
        var platform = compileConfig.alloyConfig.platform === 'ios' ? 'iphone' : compileConfig.alloyConfig.platform;
        var basefolder = path.join(compileConfig.dir.resources, platform);
        var r = require('../../lib/alloy/resolver');
        var resolver = new r({
            core: [{
                id: "alloy",
                path: "/alloy.js"
        ...
        resolver.loadFiles(basefolder);
        var registry = JSON.stringify(resolver.export(), null, 4);
        var registryImportCode = '\nmodule.exports = require("/alloy/underscore").once(function() {\n\tvar r = require("/alloy/resolver");\n\trequire("/alloy").resolve = (new r(' + registry + ').resolve);\n})();';
        fs.writeFileSync(path.join(basefolder, "alloy", "_init.js"), registryImportCode);

        var files = resolver.export().files;
        _.each(files, function(file) {
            var fullpath = path.join(basefolder, file);
